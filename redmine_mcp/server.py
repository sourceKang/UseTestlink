    ...
    if method == "initialize":
        return _result_response(
            request_id,
            {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "redmine-mcp", "version": __version__},
            },
        )
    if method == "ping":
...
def startup_health_check() -> dict[str, Any]:
    settings = load_redmine_settings()
    client = RedmineClient(settings.url, settings.api_key, timeout=settings.timeout)
...
def run(*, health_check: bool = True) -> int:
    if health_check:
        ...
        print(f"redmine-mcp v{__version__}", file=sys.stderr, flush=True)
...
def main() -> int:
    try:
        ...
        normalized = normalize_error(exc)
        print(f"redmine-mcp v{__version__} startup failed: {normalized.message}", file=sys.stderr, flush=True)
        return 1
